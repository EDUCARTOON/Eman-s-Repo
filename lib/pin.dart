...
class Pin extends StatefulWidget {
  const Pin({super.key, required this.cubit});
...
class _PinState extends State<Pin> {
  ...
  void _submitPin({required BuildContext cubitContext}) {
    String pinCode = pinList.join(""); // Convert pin list to string
    if (pinCode.length == 4) {
      UserModel profileModel = UserModel(pinCode: pinCode);
      widget.cubit.fillUserProfile(profileModel: profileModel);
                  ...
                  builder: (cubitContext, state) {
                    return SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () => _submitPin(cubitContext: cubitContext),
